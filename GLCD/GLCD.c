...
**
**------------------------------------------------------------------------------------------------------
** Modified by:			Beatrice Occhiena s314971
** Modified date:		05/01/2023
** Version:					1.0
** Descriptions:		modifiche per progetto extra point #1 tamagotchi
**									* ho anche rimosso i vari wait_delay nel caso si usi l'emulatore
**										per rendere l'animazione più fluida
**									* le funzioni scritte da me partono dalla linea 700
********************************************************************************************************/
...
//IMMAGINI IMPORTATE DA "IMAGES.c"
extern uint16_t idle1[petDIM*petDIM];
extern uint16_t battery[battW*battH];
extern uint16_t volume[volW*volH];
//-----------------------------------
extern uint16_t volume[volW*volH];
extern uint16_t line1[lineW*lineH];
extern uint16_t line2[lineW*lineH];
extern uint16_t line3[lineW*lineH];

/******************************************************************************
	FUNZIONE
	--------
		-	Disegna sul display una nuova schermata di gioco
...
void NEW_GAME(){
	...
	DRAW_Image(volX,volY,volW,volH,volSCALE,volume);
	GUI_Text(71, 13, (uint8_t *) "AGE 00:00:00", Black, White);
	LCD_DrawLine(0,40,240,40,Black);
	GUI_Text(30, 47, (uint8_t *) "Happiness", Black, White);
	GUI_Text(141, 47, (uint8_t *) "Satiety", Black, White);
	DRAW_Image(batt1_X,batt_Y,battW,battH,battSCALE,battery);
	DRAW_Image(batt2_X,batt_Y,battW,battH,battSCALE,battery);

	//Bottoni
	LCD_DrawLine(0,280,240,280,Black);
	LCD_DrawLine(119,280,119,340,Black);
...
/******************************************************************************
	FUNZIONE
	--------
		-	Disegna un'immagine su display partendo da un vettore di pixel colorati.
			L'immagine viene scalata di un fattore "scale" indicante la dimensione
			che ogni pixel deve occupare sullo schermo.
													***
			e.g. * --scale-3--> ***  (1 pixel originale -> 3x3 pixel a schermo)
													***				
		- Xpos, Ypos = coordinate iniziali su schermo.
		- W, H = largezza e altezza in pixel dell'immagine originale.

*******************************************************************************/
void DRAW_Image(uint16_t Xpos,uint16_t Ypos,uint16_t W,uint16_t H,uint16_t scale,uint16_t *IMG){
	
	uint16_t 		i, j, scale_i, scale_j;
	uint16_t		x, y;
	uint16_t 		color;
	
	//Percorro il vettore IMG con gli indici i e j.
	//Poi scalo il pixel colorato selezionato lungo x e y 
	//con i contatori scale_i e scale_j
	for(i=0; i<H; i++){
		for(scale_i=0; scale_i<scale; scale_i++){ 			
			for(j=0; j<W; j++){
				for(scale_j=0; scale_j<scale; scale_j++){	
					...
					color = IMG[i*W+j];					
					x = Xpos + j*scale + scale_j;				//aggiorno la posizione
					y = Ypos + i*scale + scale_i;				//scalata su schermo 
...
/******************************************************************************
	FUNZIONE
	--------
		-	Disegna una tacca in una delle due batterie per aggiornarne lo stato:
			*	color=White per eliminare una tacca
			* color=Black per aggiungere una tacca
			* num_batt={1,2} indica quale aggiornare

		- La nuova tacca viene disegnata a partire dal riferimento batt_X/Y della
			tacca più bassa:
			* cnt contiene il numero di tacche rimaste
...
void UPDATE_Notch(uint16_t color,uint16_t num_batt,uint16_t cnt){
	...
	if(num_batt==1)	x = notch1_X + notchW*cnt;
	else				 		x = notch2_X + notchW*cnt;
	y = notch_Y;
	
	//add non necessario se cnt già MAX
	if(color==Black && cnt==MAXnotch) return;
	
	//disegno della tacca
	for(i=0;i<notchH;i++){
		LCD_DrawLine(x,y+i,x+notchW,y+i,color);
	}
}
...
/******************************************************************************
	FUNZIONE
	--------
		-	Aggiorna a schermo le tacche del volume

*******************************************************************************/
void UPDATE_Volume(uint16_t volume){
		
		int i;
		//aggiorno il numero di tacche del volume a schermo
		for(i=0;i<=lineH*lineSCALE;i++){
			LCD_DrawLine(lineX,lineY+i,lineX+(lineW*lineSCALE)*6,lineY+i,White);
		}
		if(volume>=1)	DRAW_Image(lineX,lineY,lineW,lineH,lineSCALE,line1);
		if(volume>=2)	DRAW_Image(lineX+(lineW*lineSCALE*2),lineY,lineW,lineH,lineSCALE,line2);
		if(volume>=3)	DRAW_Image(lineX+(lineW*lineSCALE*4),lineY,lineW,lineH,lineSCALE,line3);
}
